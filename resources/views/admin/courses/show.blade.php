...
@extends('layouts.admindashboard')
    <div class="lg:ps-[260px]">
        <div class="min-h-[75rem] p-4 md:p-8">
            ...
            <!-- Add Topic Modal -->
            <div id="addTopicModal" class="hs-overlay hidden fixed inset-0 z-50 overflow-y-auto bg-gray-900 bg-opacity-50 flex justify-center items-center">
                <div class="bg-white rounded-lg shadow-lg w-full max-w-md">
                    <div class="p-4 border-b flex justify-between">
                        <h5 class="text-lg font-semibold">Select a Topic to Add</h5>
                        <button type="button" class="text-gray-500 hover:text-gray-700" onclick="document.getElementById('addTopicModal').classList.add('hidden')">✕</button>
                    </div>
                    <div class="p-4">
                        <form action="{{ route('admin.courses.addTopic', $course->id) }}" method="POST">
                            ...
                            <button type="submit" class="mt-4 w-full px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">Add Topic</button>
                        </form>
                    </div>
                </div>
            </div>
